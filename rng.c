...
void rng_gauss(long double *grn1, long double *grn2) {
  /*// Box–Muller method
  ...
  void rn_ab(long double *, long double *, long double *);
  do {
   rn_ab(&a, &b, &u); rn_ab(&a, &b, &v); s = powl(u,2)+powl(v,2);
 } while ( s >= 1 || s == 0);
  w = sqrtl(-2.0*logl(s)/s);
  (*grn1) = u*w; (*grn2) = v*w;
}

void rng_exp(long double *ern) {
  double genrand64_real1();
  long double rn = 0.0;
  while (rn == 0.0) rn = ((long double)genrand64_real1());
  *(ern) = -logl(rn);
}

void rn_ab(long double *a, long double *b, long double *rn) {
  double genrand64_real1();
  (*rn) = (*a) + ((*b)-(*a))*((long double) genrand64_real1());
}
...
void test_gauss() {
  ...
  long double xmin = -5.0, xmax = 5.0;
  long double delta =  (xmax-xmin)/((long double) ni);
  int ct[ni];
  for(j = 0; j < ni; j++){
    ct[j] = 0;
  }
  ...
  int s = 0;
  for (l = 0; l < (ni-1); l++) { s += ct[l]; }; printf("%Lf \n",(long double)s/(2*(long double)ns));
  FILE *fd = fopen("plot.dat", "w");
  for(l = 0; l < (ni-1); l++){
    fprintf(fd, "%Lf %Lf \n", (xmin +(l+0.5)*delta), (long double)ct[l]/(delta*(long double)(2*ns)));
  }
  fclose(fd);
  void plot(); plot();
}
...
void test_exp() {
  int j, l, ns = pow(10,6), ni = 200;
  long double xmin = 0.0, xmax = 5.0;
  long double delta = (xmax-xmin)/((long double) ni);
  int ct[ni];
  for(j = 0; j < ni; j++){
    ct[j] = 0;
  ...
  printf("%Lf \n",((long double) s)/((long double) ns));
  FILE *fd = fopen("plot.dat", "w");
  for(l = 0; l < (ni-1); l++){
    fprintf(fd, "%Lf %Lf \n", (xmin +(l+0.5)*delta), ((long double) ct[l])/(delta*ns));
  }
  fclose(fd);
  void plot(); plot();
}
...
void test_rand_int() {
  int j, ns = pow(10,6), a, b, ri, rn_int_ab(int *, int *);
